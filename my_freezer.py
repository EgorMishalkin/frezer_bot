...
def add_user(conn, user_id: int, freezer, add_or_not: int, del_or_not: int):
    c = conn.cursor()
    c.execute('INSERT INTO users (user_id, freezer, add_or_not, del_or_not) VALUES (?, ?, ?, ?)',
              (user_id, freezer, add_or_not, del_or_not))
    conn.commit()
    ...
    try:
        if get_user[0][0]:
            bot.send_message(message.chat.id,
                             'Приветствую, ' + str(message.from_user.first_name) + '. Вы уже существуете в базе',
                             reply_markup=keyboard1)
        else:
            add_user(user_id=message.from_user.id, freezer='', add_or_not=0, del_or_not=0)
            bot.send_message(message.chat.id, str(message.from_user.first_name) + ', регистрация прошла успешно!',
                             reply_markup=keyboard1)
    except IndexError:
        add_user(user_id=message.from_user.id, freezer='', add_or_not=0, del_or_not=0)
        bot.send_message(message.chat.id, str(message.from_user.first_name) + ', регистрация прошла успешно!',
...
def gettext(message):
    ...
    # тут происходит обработка удаления
        ...
        try:
            # c.execute(f'UPDATE users SET freezer = "{a}" WHERE user_id = {user}')
            print(mess in freezer)
            print(freezer == mess)
            print(mess)
            print(user)
            c.execute(f'DELETE FROM users WHERE  freezer = "{mess}" AND user_id = {int(user)}')
            # query = f'DELETE FROM users WHERE "{mess}" IN freezer AND user_id = {user}'
            # print(query)
            # c.execute(query)
            conn.commit()
            # mess = 'банан 4'
